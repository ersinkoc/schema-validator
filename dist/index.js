...
const defaultErrorMap = (ctx) => {
    switch (ctx.code) {
        case exports.ErrorCode.INVALID_TYPE:
            return `Expected ${ctx.expected}, received ${ctx.received}`;
        case exports.ErrorCode.INVALID_LITERAL:
            return `Invalid literal value, expected ${JSON.stringify(ctx.expected)}`;
        case exports.ErrorCode.UNRECOGNIZED_KEYS:
            return `Unrecognized key(s) in object: ${ctx.options?.['keys']?.join(', ')}`;
        case exports.ErrorCode.INVALID_UNION:
            return 'Invalid input';
        case exports.ErrorCode.INVALID_UNION_DISCRIMINATOR:
            return `Invalid discriminator value. Expected ${ctx.options?.['expected']?.join(' | ')}`;
        case exports.ErrorCode.INVALID_ENUM_VALUE:
            return `Invalid enum value. Expected ${ctx.options?.['expected']?.join(' | ')}, received '${ctx.received}'`;
        case exports.ErrorCode.INVALID_ARGUMENTS:
            return 'Invalid function arguments';
        case exports.ErrorCode.INVALID_RETURN_TYPE:
            return 'Invalid function return type';
        case exports.ErrorCode.INVALID_DATE:
            return 'Invalid date';
        case exports.ErrorCode.INVALID_STRING:
            return ctx.message || 'Invalid string';
        case exports.ErrorCode.TOO_SMALL:
        ...
        case exports.ErrorCode.TOO_BIG:
            if (ctx.options?.['type'] === 'array') {
        ...
        case exports.ErrorCode.NOT_MULTIPLE_OF:
            return `Number must be a multiple of ${ctx.options?.['multipleOf']}`;
        case exports.ErrorCode.NOT_FINITE:
            return 'Number must be finite';
        case exports.ErrorCode.INVALID_REGEX:
            return 'Invalid regular expression';
        case exports.ErrorCode.INVALID_EMAIL:
            return 'Invalid email';
        case exports.ErrorCode.INVALID_URL:
            return 'Invalid URL';
        case exports.ErrorCode.INVALID_UUID:
            return 'Invalid UUID';
        case exports.ErrorCode.INVALID_CUID:
            return 'Invalid CUID';
        case exports.ErrorCode.INVALID_DATETIME:
            return 'Invalid datetime string';
        case exports.ErrorCode.INVALID_IP:
            return `Invalid IP address (version ${ctx.options?.['version'] || 'unknown'})`;
        case exports.ErrorCode.INVALID_JSON:
            return 'Invalid JSON';
        case exports.ErrorCode.INVALID_BASE64:
            return 'Invalid base64 string';
        case exports.ErrorCode.REQUIRED:
            return 'Required';
        case exports.ErrorCode.CUSTOM:
        default:
...
class ObjectSchema extends BaseSchema {
    ...
    _parse(input, ctx) {
        ...
        if (parsedType !== 'object') {
            ctx.addIssue({
                code: exports.ErrorCode.INVALID_TYPE,
                expected: 'object',
                received: parsedType
            });
            throw ctx.makeError();
        ...
        if (unknownKeys.length > 0) {
            if (this._unknownKeys === 'strict') {
                ctx.addIssue({
                    code: exports.ErrorCode.UNRECOGNIZED_KEYS,
            ...
            switch (check.kind) {
                case 'min':
                    if (arr.length < check.value) {
                        ctx.addIssue({
                            code: exports.ErrorCode.TOO_SMALL,
                ...
                case 'max':
                    if (arr.length > check.value) {
                        ctx.addIssue({
                            code: exports.ErrorCode.TOO_BIG,
                            message: check.message || 'Array validation failed',
                            options: {
                                type: 'array',
                                maximum: check.value,
...
class UnionSchema extends BaseSchema {
    ...
    _parse(input, ctx) {
        ...
        ctx.addIssue({
            code: exports.ErrorCode.INVALID_UNION,
        ...
        for (const check of this._checks) {
            switch (check.kind) {
                ...
                case 'length':
                    if (value.length !== check.value) {
                        ctx.addIssue({
                            code: exports.ErrorCode.INVALID_STRING,
                ...
                case 'email':
                    if (!isValidEmail(value)) {
                        ctx.addIssue({
                            code: exports.ErrorCode.INVALID_EMAIL,
                            message: check.message
                        });
                    }
                    break;
                case 'url':
                    if (!isValidUrl(value)) {
                        ctx.addIssue({
                            code: exports.ErrorCode.INVALID_URL,
                            message: check.message
                        });
                    }
                    break;
                case 'uuid':
                    if (!isValidUuid(value)) {
                        ctx.addIssue({
                            code: exports.ErrorCode.INVALID_UUID,
                            message: check.message
                        });
                    }
                    break;
                case 'cuid':
                    if (!isValidCuid(value)) {
                        ctx.addIssue({
                            code: exports.ErrorCode.INVALID_CUID,
                            message: check.message
                        });
                    }
                    break;
                ...
                case 'datetime':
                    if (!isValidDatetime(value)) {
                        ctx.addIssue({
                            code: exports.ErrorCode.INVALID_DATETIME,
                            message: check.message
                        });
                    }
                    break;
                case 'ip':
                    ...
                    if (!isValidIp(value, version)) {
                        ctx.addIssue({
                            code: exports.ErrorCode.INVALID_IP,
                            message: check.message,
                            options: { version }
                        });
                    }
                ...
                case 'base64':
                    if (!isValidBase64(value)) {
                        ctx.addIssue({
                            code: exports.ErrorCode.INVALID_BASE64,
                            message: check.message
                        });
                    }
                    break;
            ...
            switch (check.kind) {
                ...
                case 'multipleOf':
                    if (value % check.value !== 0) {
                        ctx.addIssue({
                            code: exports.ErrorCode.NOT_MULTIPLE_OF,
                            message: check.message,
                            options: {
                ...
                case 'finite':
                    if (!Number.isFinite(value)) {
                        ctx.addIssue({
                            code: exports.ErrorCode.NOT_FINITE,
                            message: check.message
                        });
                    }
                    break;
...
class DateSchema extends BaseSchema {
    ...
    _parse(input, ctx) {
        ...
        if (isNaN(value.getTime())) {
            ctx.addIssue({
                code: exports.ErrorCode.INVALID_DATE,
...
class LiteralSchema extends BaseSchema {
    ...
    _parse(input, ctx) {
        if (input !== this._value) {
            ctx.addIssue({
                code: exports.ErrorCode.INVALID_LITERAL,
    ...
    _parse(input, ctx) {
        ...
        if (this._rest === null) {
            if (arr.length !== this._items.length) {
                ctx.addIssue({
                    code: arr.length > this._items.length ? exports.ErrorCode.TOO_BIG : exports.ErrorCode.TOO_SMALL,
...
class DiscriminatedUnionSchema extends BaseSchema {
    ...
    _parse(input, ctx) {
        ...
        if (!option) {
            ctx.addIssue({
                code: exports.ErrorCode.INVALID_UNION_DISCRIMINATOR,
...
class EnumSchema extends BaseSchema {
    ...
    _parse(input, ctx) {
        if (!this._enumValues.has(input)) {
            ctx.addIssue({
                code: exports.ErrorCode.INVALID_ENUM_VALUE,
